    ...
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> List()
        {
            var model = await _dataContext.Products
                ...
                p.ProductImages.Where(p => p.IsPoster == false).Take(1).FirstOrDefault() != null
                ? _fileService.GetFileUrl(p.ProductImages.Where(p=> p.IsPoster == false).Take(1).FirstOrDefault().ImageNameFileSystem,UploadDirectory.Product)
        ...
        //        Price = product.Price,
        //        PosterImg = await _dataContext.ProductImages.Select(P=> new PosterImg(P.Id,P.ImageNameFileSystem)).ToListAsync(),
